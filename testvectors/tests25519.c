...
// Then copy https://github.com/C2SP/wycheproof/blob/master/testvectors/eddsa_test.json into the work directory
// Move to the work directory and process the test vectors into a form digestible by simple C code
//
// python3 parse.py eddsa_test.json > tvs25519.txt
//
...
{       
    ...
    char buff[4096],pub[BYTES];
    char m[4096],sig[2*BYTES];

    fp=fopen("tvs25519.txt","rt");
    ...
    while(fgets(buff, 4096, fp)) {
	    ...
	    res=ED25519_VERIFY(pub,lenm,m,sig);
