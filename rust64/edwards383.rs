...
fn flatten(n: &mut [SPINT]) -> SPINT {
    ...
    n[6] += (0x10400000000000 as SPINT) & (carry as SPINT);
    prop(n);
    return carry & 1;
}
...
fn modadd(b: &[SPINT], n: &mut [SPINT]) {
    ...
    n[6] = n[6] + b[6];
    n[0] += 2;
    n[6] -= 0x20800000000000 as SPINT;
    let carry = prop(n);
    n[0] -= 2 & (carry as SPINT);
    n[6] += (0x20800000000000 as SPINT) & (carry as SPINT);
    prop(n);
    return;
}
...
fn modmli(b: usize, c: &mut [SPINT]) {
    let p6 = 0x10400000000000 as SPINT;
    let mask = ((1 as SPINT) << 55) - 1;
...
const constant_b: [SPINT;7]=[0,0,0,0,0,0,0];
const CONSTANT_X: usize = 2;
const constant_x: [SPINT;7]=[0,0,0,0,0,0,0];
const constant_y: [SPINT;7]=[0,0,0,0,0,0,0];
